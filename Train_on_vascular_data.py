...
if(spectral_flag):
    force_model_path = f"/mnt/ceph/users/smaddu/stochastic_inference/fm_newset_fullspectral_vascular_N{nsamples}_Ndim{Ndim}_Np{Np}_Nf{Nf}_fac{fac}_seed{seed}_dm{dm}_lx{lx}_set{geneset_num}.path"
    torch.save(drift_net_FM_cheb, force_model_path)   
else:
    force_model_path = f"/mnt/ceph/users/smaddu/stochastic_inference/fm_newset_vascular_N{nsamples}_Ndim{Ndim}_Np{Np}_Nf{Nf}_fac{fac}_seed{seed}_dm{dm}_lx{lx}_set{geneset_num}.path"
    torch.save(drift_net_FM_cheb, force_model_path)      
