...
def train_simulation_free(samples, Ndim, Nf, nsamples, tp, model_params_, fac, device, nmb, interp='cheb',dm='Langevin'):
    ...
    if(prescient_flag):
        ...
        X_mean = torch.zeros((Ndim+1,)); X_std = torch.ones((Ndim+1,))
        for i in range(Ndim):
            X_mean[i] = torch.tensor(np.mean(Dist[:,:,i]), dtype=torch.float32)
            X_std[i]  = torch.tensor(np.std(Dist[:,:,i]), dtype=torch.float32)
    
        X_mean[Ndim] = torch.tensor(np.mean(samples[:,:,Ndim+1]), dtype=torch.float32)
        X_std[Ndim] = torch.tensor(np.std(samples[:,:,Ndim+1]), dtype=torch.float32)
    ...
    if prescient_flag:
        kwargs["sizes"] = [Ndim, Nf, Nf, Nf, Nf, Nf, 1]
    else:
        kwargs["sizes"] = [Ndim + 1, Nf, Nf, Nf, Nf, Ndim]
    drift_net_FM = cls(**kwargs).to(device)

    print(" neural network ", drift_net_FM)    
    ...
    for epoch in range(n_epochs):
        ...
        if(prescient_flag):
            ...
            inp_.requires_grad_()
            phi = drift_net_FM(inp_)
            grad_drift  = gradient_field(phi, inp_)   
            fold = -grad_drift - lx*Xtrain[:,0:Ndim] - (kappa**2)*scoreNet(Xtrain[:,:])
        else:
            inp_ = Xtrain[:, torch.cat([torch.arange(0, Ndim), torch.tensor([Ndim+1])])].clone()
            inp_.requires_grad_()
            fold = drift_net_FM(inp_) -  lx*Xtrain[:,0:Ndim]
...
parser_param.add_argument('--prescient_flag', type=str2bool, required=True,
                    help='True: if you like to run the prescient model')
...
prescient_flag = args.prescient_flag
lx = args.lx
...
Dist, tp, ind_array, cell_types = load_data(path,nsamples,genes,"day","annot_celltype",seed)
print(cell_types)
...
if(spectral_flag):
    model_path = f"/mnt/ceph/users/smaddu/stochastic_inference/score_newset_spectral_vascular_general_N{nsamples}_Ndim{Ndim}_Np{Np}_seed{seed}_set{geneset_num}.pth"
else:
    model_path = f"/mnt/ceph/users/smaddu/stochastic_inference/score_newset_vascular_general_N{nsamples}_Ndim{Ndim}_Np{Np}_seed{seed}_set{geneset_num}.pth"
    
if os.path.exists(model_path):
    print(f"Model already exists at {model_path}, skipping training.")
    scoreNet = torch.load(model_path,weights_only=False)
...
# Construct model tag based on flag combination
if spectral_flag and prescient_flag:
    model_tag = "fullspectral_prescient"
elif spectral_flag and not prescient_flag:
    model_tag = "fullspectral_TrajNet"
elif not spectral_flag and prescient_flag:
    model_tag = "prescient"
else:
    model_tag = "TrajNet"
...
# Build path
force_model_path = (
    f"/mnt/ceph/users/smaddu/stochastic_inference/fm_newset_{model_tag}_vascular"
    f"_N{nsamples}_Ndim{Ndim}_Np{Np}_Nf{Nf}_fac{fac}"
    f"_seed{seed}_dm{dm}_lx{lx}_set{geneset_num}.path"
)
